...
{
    public partial class FormPnET : Form
    {
        SiteData Sitedata = new SiteData();  // Instance of the sitedata class 
                
        public string InputPnETSuccession
        {
            get => checkBoxPnETCN.Checked ? "PnET-CN-Succession" : "PnET-Succession";
        }
        private string InterDirectory
        
        {
                get { return InterDir(InputPnETSuccession); }
         }
        private string InputDirectory
        {
            get { return InputDir(InputPnETSuccession); }
        }
        ...
        {
            //string InputDirectory = InputDir(InputSuccession);// get the current succesion input directory            
        ...
        private void InitializeCbEcoPara()
        {
            ...
            string filePath = Path.Combine(InterDirectory, fileName);  // Path to the text file
        ...
        private void InitializeCbSppGenericPara()
        {
            ...
            // read the data file
            string fileName = "Site_Generic_Pnet.txt";
            string filePath = Path.Combine(InterDirectory, fileName);  // Path to the text file
        ...
        private void LoadResultSiteRep(string sitename = "Site.csv")
        {
            ...
            string InputSuccession = InputPnETSuccession;
            ...
            for (int i = 0; i < repnum; i++)
            {
                ...
                string filePath = RepDirectory + OutputFolder;
                if (InputSuccession == "PnET-Succession")
                {
                    filePath = RepDirectory + OutputFolder;
                }
            ...
            //string filePath2 = InterDir(InputPnETSuccession);
            string filePath2 = OutputDirectory + "\\output.csv";

            SaveToCsv(rowSums, filePath2);  // save the aggregated results and plot it
        ...
        private void LoadRecordsCalOne(string OutputDirectory, string sitename = "Site.csv")
        {
            ...
            OutputDirectory2 = OutputDirectory + OutputFolder;
        ...
        public void SaveInputSite(string filePath = @"Inter\Site_input.csv")
        {
            ...
            string SuccessionOption = "PnET-Succession"; // set for different succession extension
            string Fieldname = " ";
            ...
            using (StreamWriter writer = new StreamWriter(filePath))
            {
                ...
                string strtemp = "PnET-Succession";
                
             /*   if (checkBoxPnETCN.Checked) 
                {
                    strtemp = "PnET-CN-Succession";
                }
             */
                writer.WriteLine("SuccessionExtension," + SuccessionOption);

                ///////////////// Additional Extensions
                writer.WriteLine(strseperater + "Additional Extensions");
                strtemp = checkBoxPnETCN.Checked ? "Yes" : "No";
                writer.WriteLine("PnETCNCheck," + strtemp);
                ...
                foreach (var item in checkedListBoxOutputExt.CheckedItems)
                {
                    Fieldname = "OutputExtension";
                    string value = item.ToString();

                    int index = checkedListBoxOutputExt.Items.IndexOf(value);
                    //int index = item.IndexOf(value);
                    string strout = Fieldname + index.ToString() + "," + value;
                    writer.WriteLine(strout);
                }
        ...
        public void LoadInputFromCsv(string filePath)
        {
            try
            {
                ...
                values = lines[4].Split(','); // succession extension
                checkBoxPnETCN.Checked = values[1].Equals("Yes"); 
                ...
                // output extention
                 for (int ii = 0; ii < checkedListBoxOutputExt.Items.Count; ii++) checkedListBoxOutputExt.SetItemChecked(ii, false);
                for (int i = startline; i < lines.Length; i++)
                {
                    if (lines[i].Contains(searchPhrase))
                    {
                    ...
                    values = lines[i].Split(',');
                    index = checkedListBoxOutputExt.Items.IndexOf(values[1]);
                    if (index >= 0)
                    {
                        checkedListBoxOutputExt.SetItemChecked(index, true);
                    }                    
                }
        ...
        private string InputDir(string InputSuccession = "PnET-Succession")
        {
        ...
        private string InterDir(string InputSuccession = "PnET-Succession")
        {
        ...
        private void BuildLandisInput()
        {
            ...
            if (InputPnETSuccession == "PnET-Succession") BuildPnetSuccession();
            if (InputPnETSuccession == "PnET-CN-Succession") BuildPnetSuccession();            
        ...
        private void BuildRunBatch()
        {
           ...
           // string fileName = "site_run_v7.bat";   //// ZZX Testing v7
           ...
           // string InterDirectory = @".\Inter";
            ...
            //fileName = "site_run_console_v7.bat"; //// ZZX Testing v7
        ...
        private void BuildPnetSuccessionSuccession()
        {
        ...
        private void BuildPnetSuccessionSpecies()
        {
        ...
        private void BuildPnetSuccessionPnETSpeciesParameters()
        {
        ...
        private void BuildPnetSuccessionInitialComm()
        {
        ...
        private void BuildPnetSuccessionClimate()
        {
        ...
        private void BuildPnetSuccessionEcoregionPara()
        {
        ...
        private void BuildPnetSuccessionGenericPara()
        {
        ...
        private void BuildPnetSuccessionRemain()
        {
            List<string> items = new List<string>{ "site_PNEToutputsites.txt",    // output file
                "site_Ecoregions.txt",                                             // ecoregion file, it is fixed as the model is in site mode
                "site_EcoregionsMap.img",                                          // the onecell ecoregion map
                "site_InitialCommunitiesMap.img"                                    // the default communities map
            };
        ...
        private void BuildPnetSuccession()
        {
            
            BuildRunBatch();  // build the running batch file
            BuildSuccessionScenario();                        // build scenario file
            BuildPnetSuccessionSuccession();                      // build Succession file
            BuildPnetSuccessionSpecies();                         // build Species file
            BuildPnetSuccessionPnETSpeciesParameters();           // build PnET Species file
            BuildPnetSuccessionInitialComm();                     // build initial community file
            BuildPnetSuccessionClimate();                         // build climate file
            BuildPnetSuccessionEcoregionPara();                 // build ecoregion file
            BuildPnetSuccessionGenericPara();                     // build PnETGeneric file
            BuildPnetSuccessionRemain();                            // copy other remaining files from inter folder to input folder
        ...
        private void checkBoxPnETCN_CheckedChanged(object sender, EventArgs e)
        {
           // if (checkBoxPnETCN.Checked == true) InputPnETSuccession = "PnET-CN-Succession"; 
           // else InputPnETSuccession = "PnET-Succession"; 
        }
        // for Random seed
        ...
        private void tabControlGraphSitePnET()
        {
            ...
            // read the data file
            //string InterDirectory = InterDir(InputPnETSuccession);// get the current succesion inter directory
 ...
 //           string InputSuccession = InputPnETSuccession;
            ...
            OutputDirectory = OutputDirectory + OutputFolder;
            return OutputDirectory;

        }

        private string OutputParentDir(Boolean control = true)
        {
        ...
        private void comboBoxCohortVarFill()
        {
           ...
           // comboBoxCohortName.SelectedIndex = 0;
            string OutputDirectory = OutputDir();// get the current succesion output directory
            string fileName = comboBoxCohortName.Items[0].ToString(); // first file
            ...
            comboBoxCohortVar.SelectedIndex = 0;  // set default
