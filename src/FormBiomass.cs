...
namespace LANDIS_II_Site
{
    ...
    public partial class FormBiomass : Form
    {
        ...
        private void InitializeComponentPlus()
        {
            ...
            // disable and hide the climate tab
            SetClimateTabVisible(tabPageClimate, false);
            SetClimateTabVisible(tabPageWater, false);
            SetClimateTabVisible(tabPageCohorts, false);
            SetClimateTabVisible(tabPageCompare, false);
...
*/
        ...
        // Hide or restore the climate tab and disable it when hidden
        private void SetClimateTabVisible(TabPage tabPage, bool visible)
        {
            if (tabControlGraph == null || tabPage == null)
                return;

            if (visible)
            {
                if (!tabControlGraph.TabPages.Contains(tabPage))
                    tabControlGraph.TabPages.Add(tabPage);
                tabPage.Enabled = true;
            }
            else
            {
                // mark disabled so logic relying on Enabled sees it's off
                tabPage.Enabled = false;
                // remove from TabControl so it's not visible
                if (tabControlGraph.TabPages.Contains(tabPage))
